...
def compile_dual_plan(path: str | Path, base_features: list[FeatureSpec], config: ScanConfig) -> list[CompiledDualFeature]:
    ...
    for item in raw.get("definitions", []):
        ...
        canonical=(definition.operator,tuple(sorted((definition.feature_a,definition.feature_b))) if definition.operator in {"intersection","aligned_rank_mean"} else (definition.feature_a,definition.feature_b),_canonical({"a":definition.condition_a,"b":definition.condition_b,"ta":definition.transform_a,"tb":definition.transform_b}))
