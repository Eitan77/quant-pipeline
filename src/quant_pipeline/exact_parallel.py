...
def exact_pair(
    ...
    target_frame=pd.read_parquet(target_path,columns=["symbol","session_date","bar_start_ts","decision_ts",target])
    ...
    if row is not None:
        ...
        else:row.update(_confirmation_gate(float(row.get("top_bottom_spread",np.nan)),{},np.sign(direction_hint or row.get("spearman",0)),config))
        row.update(_walk_forward(frame.loc[eligible],spec.name,target,direction_hint or row.get("spearman",0)))
        if row.get("confirmation_status")=="statistically_confirmed" and row.get("confirmation_robust") and row.get("walk_forward_positive_fold_pct",0)>=config.confirmation_min_positive_fold_fraction:row["confirmation_status"]="fully_confirmed_phase1"
