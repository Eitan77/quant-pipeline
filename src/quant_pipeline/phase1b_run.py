...
def run_phase1b(source_run: str | Path, config: ScanConfig) -> Path:
    ...
    raw_p=pd.to_numeric(dual.get("raw_p",pd.Series(index=dual.index,dtype=float)),errors="coerce")
    valid_phase1b_result_count=int(raw_p.notna().sum())
    if built_dual_feature_count>0 and valid_phase1b_result_count==0:
        raise RuntimeError("Phase 1B features were built, but no feature-target pair produced valid statistical inference.")
    ...
    promotion_ready=built_dual_feature_count>0 and valid_phase1b_result_count>0 and expected.issubset(observed)
