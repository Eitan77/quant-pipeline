...
from quant_pipeline.interday.corporate_actions import calculate_holding_outcome, normalize_actions, vectorized_total_return
...
def test_vectorized_actions_are_scoped_by_security():
    actions=normalize_actions(pd.DataFrame([{"symbol":"X","effective_session":"2025-01-02","action_type":"cash_dividends","cash_amount":1}]))
    total, price=vectorized_total_return(np.array([[10.0,10.0]]),np.array([[10.0,10.0]]),np.array([[pd.Timestamp("2025-01-01")]*2]),np.array([[pd.Timestamp("2025-01-03")]*2]),actions,security_ids=np.array(["x","y"]),symbols=np.array(["X","Y"]))
    assert total[0,0] > price[0,0] and total[0,1] == price[0,1]
